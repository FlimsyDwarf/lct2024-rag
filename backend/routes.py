...
def make_request():
    ...
    amount_of_links = data.get('amountOfLinks', 5)
    ...
    report_generator = ReportGenerator(
        use_internet, user_sources, date_range, suffix, amount_of_links)
...
@api.route('/download_template', methods=['POST'])
def download_template():
    data = request.form
    id = data.get('id', '')
    return send_file(os.path.join(UPLOAD_FOLDER_DOCS, id + "template.docx"),
                     as_attachment=True,
