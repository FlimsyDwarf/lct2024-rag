...
os.environ['TAVILY_API_KEY'] = "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"
os.environ['OPENAI_API_KEY'] = "YOUR_OPENAI_API_KEY"
tavily_keys = ["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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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",
               "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"]


class Pipeline:

    def __init__(self, use_internet: bool, user_sources: str, date_range: str, suffix: str, amount_of_links: int):
    ...
    def generate_block(self, query: str) -> dict:
        ...
        result = {}
        for q in tqdm(list_of_questions["questions"], desc="WebSearching", postfix="\n"):
            print(f'\n\nПоиск ответа на вопрос: {q}')

            documents = retriever.invoke(q)
            inputs = {"question": q, "documents": documents,
                      "generation_count": 3, "use_internet": self.use_internet, "amount_of_links": self.amount_of_links}
...
def web_search_node(state):
    ...
    amount_of_links = state["amount_of_links"]

    # Web search
    if use_internet:
        print("---WEB SEARCH---")
        web_search_tool = TavilySearchResults(max_results=amount_of_links)
        ...
        while docs == "HTTPError('400 Client Error: Bad Request for url: https://api.tavily.com/search')":
            ...
            if len(tavily_keys) == i:
                print(
                    "You must update the Tavily API-KEY, all preset have reached their limits. "
                    "Internet search is no longer possible.")
                break
        for doc in docs:
